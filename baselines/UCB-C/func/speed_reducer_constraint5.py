...
class SpeedReducerConstraint5(BaseFunc):
	def __init__(
		self,
		xsize=100,
		zsize=100,
		...
		super(SpeedReducerConstraint5, self).__init__(
			xdim, zdim, xsize, zsize, transformation, noise_std=noise_std
		)

		self.module_name = "speed_reducer_constraint5"
		self.xsize = xsize
		self.zsize = zsize
	...
	def get_noiseless_observation(self, xz):
		with torch.no_grad():
			...
			val = (
				1/ (0.1 * x6.pow(3))* torch.sqrt((745 * x4 / (x2 * x3)).pow(2) + 16.9 * 1e6) - 1100
		  	).reshape(
				-1,
			)
