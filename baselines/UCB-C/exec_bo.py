...
from opt_bo import MultiConstraintBO, ACQUISITION_FUNCTIONS
import my_importlib
import torch
...
parser.add_argument(
    ...
    dest="saveregret",
    type=int,
    help="1 if the regret is saved to file, 0 otherwise",
)
parser.add_argument(
    "--initseed",
    dest="initseed",
...
parser.add_argument("--path", type=str, default='/home/trongp/constrained_NeuralBO/baselines/UCB-C/expm/sensitive_sample.json', help="Path to experiments config")
parser.add_argument(
    "--acqfunc",
...
is_visualize = bool(args.isplot) if args.isplot is not None else False

is_regret_save = bool(args.saveregret) if args.saveregret is not None else True
...
constraint_list = [
    {
        ...
        if ("fix_mean_at" in data["target"])
        else None,
    }
    for constraint_info in data["constraints"]
]

save_root = f"/home/trongp/constrained_NeuralBO/results/{data['target']['identifier']}_DIM_{data['dimension']}_ITERS_{data['n_bo_iterations']}/{acquisition_function_name.upper()}"
...
for nr in range(nrand[0], nrand[1]):
    ...
    for i, func_info in enumerate([target] + constraint_list):
        ...
        func_info["init_xz"], func_info["init_y"] = func_info[
            "func"
        ].get_init_observations(func_info["n_init_observations"], seed=initseed + nr)
    ...
    results_filename = f"{save_root}/{acquisition_function_name.upper()}_{data['target']['identifier']}_dim{data['dimension']}.{nr}.pkl"
    ...
    # if is_regret_save:
    #     print("Save regret and BO information")
